...
def get_response(instructions, previous_questions_and_answers, new_question, freq_pen, file_path):
    """Get a response
...
def get_compatibility(input_bio_1, input_bio_2, MAX_CONVO_LENGTH=10):
    ...
    for _ in range(MAX_CONVO_LENGTH):
        ...
        response1 = get_response(person1.gpt_instruction, previous_questions_and_answers_person_1, latest_question, person1.frequency_penalty, None)
        ...
        # Person 2 responds to last q        # Latest question is the response of the 1st personuestion
        response2 = get_response(person2.gpt_instruction, previous_questions_and_answers_person_2, latest_question, person2.frequency_penalty, None)
    ...
    judge_response = get_response(full_conversation+"""Evaluate the conversation: how compatible are these 2 people? Highlight any potential issues that if they were roommates would arise. Be quite harsh and definitely highlight any red flags. Be harsh but fair.""",
                 [], "Return it as just a tuple that can be parsed by ast in python where tuple[0] is the string describing potential issues, and tuple[1] is a float for the number from 0 to 100 scale where 0 is very bad compatibility and 100 is full compatibility.", 1.0, None)
...
def make_bio(transcript):
    transcript_analysis_instructions = f"""Analyze this transcript of a potential roommate's audio introduction and provide a comprehensive breakdown in the following categories: Basic Information

    Extract all personal details (age, year of study, major, hometown)
    Living preferences (sleep schedule, noise tolerance, visitors policy)
    Lifestyle habits (cleanliness, smoking/drinking, gym/exercise routines)

Personality Assessment

    Identify communication style (direct/indirect, formal/casual, verbose/concise)
    Note emotional expressions and tone (enthusiasm, humor, seriousness)
    Evaluate sociability indicators (mentions of parties, social activities, alone time)
    Assess study habits and academic focus
    Flag any mentioned hobbies, interests, or extracurricular activities

Living Style Indicators

    Time spent in room/apartment
    Mentioned routines or schedules
    Social habits (hosting friends, quiet time preferences)
    Academic workspace preferences

Red Flags & Green Flags

    Note any strong preferences or deal-breakers
    Identify potential compatibility factors
    Highlight any unusual requests or requirements

Personality Profile
Create a brief personality summary addressing:

    Introvert/Extrovert tendencies
    Academic vs. social life balance
    Conflict resolution style (if mentioned)
    General maturity level
    Flexibility and adaptability indicators

Present the analysis in a structured format with specific examples from the transcript to support each observation. Maintain objectivity and avoid making assumptions beyond what's directly stated or strongly implied in the text, while still inferring as much as you can. If a section is empty, write that there are no preferences indicated. The transcript is as follows: {transcript}"""
   
    transcript_analysis = get_response("Act like someone completely impartial and professional, who is an expert at construction biographies for people and making inferences", [], transcript_analysis_instructions, 1, None)
    return transcript_analysis
    #for curFile in additionalFilePaths:
        #transcript_analysis = get_response("Please await instructions", transcript_analysis_instructions, [], 1)
...
if __name__ == "__main__":
    ...
    bio = make_bio(text)
    print(get_compatibility(bio, example_input_bio_1, 5))